...
class CornersProblem(search.SearchProblem):
    ...
    """
    #we represent the state as ( (x,y), ((x,y),(x,y),(x,y),(x,y)) )
    #where the state[0] is the position of state
    #and state[1] the list of tuples with the positions of unvisted corners for that specific state[1]
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.startingPosition, self.corners) #the start-state wont have visisted a corner yet
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return len(state[1]) == 0 #our goal state is when we have visited all corners
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            children.append(((nextState,tuple(unvisitedCorners)),action,self.getActionCost(state,action,nextState)))
...
def cornersHeuristic(state, problem):
    ...
    if(unvisitedCorners):
        for corner in unvisitedCorners:
            cost.append(util.manhattanDistance(state[0],corner))
    return max(cost)
...
def foodHeuristic(state, problem):
    ...
    if foodList :
        for food in foodList :
            ...
            cost.append(problem.heuristicInfo[(position,food)])
    return max(cost)
